class Track {
  ...
  set Name(value) {
    if (typeof value !== "string")
      throw TypeError(`name track is not a string`);
  ...
  set Image(value) {
    if (typeof value !== "string")
      throw TypeError(`image track is not a string`);
  ...
  set Preview_url(value) {
    if (typeof value !== "string")
      throw TypeError(`preview_url track is not a string`);
    if (!value.trim().length)
      throw Error(`preview_url track is empty or blank`);
...
class Playlist {
  ...
  set Id(value) {
    if (typeof value !== "string")
      throw TypeError(`id playlist is not a string`);
  ...
  addTrack(track) {
    if (track && track.Id && typeof track.Id === "string") {
      ...
      if (!this.findTrack(track.Id)) this.tracks.push(track);
      else throw Error("The track exists in the playlist");
    } else throw Error("Track object is invalid");
  }
  ...
  deleteTrack(trackId) {
    ...
    if (this.tracks.find(el => el.id === trackId)) {
      ...
      return true;
    } else throw Error("The track not exists in the playlist");
  }
}
...
class User {
  ...
  createPlayList(playList) {
    const isNameAlreadyTaken = this.playLists.find(
      el => el.name === playList.Name
    );
    if (isNameAlreadyTaken)
      throw Error(`A playlist with the name${playList.Name} already exists `);
  ...
  deletePlayList(playListId) {
    if (typeof playListId !== "string")
      throw TypeError(`playListId is not a string`);
  ...
  existsTrackInPlayList = trackId => {
    ...
    if (this.playLists.length) {
      ...
      return found;
    } else throw Error("The user has no playlists");
  };
  ...
  deleteTrackFromPlayList(trackId) {
    ...
    } else throw Error("The track does not belong to any playList");
  }
}
