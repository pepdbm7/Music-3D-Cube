...
export default class List extends Component {
  ...
  componentWillReceiveProps(props) {
    this.setState({
      hiddePlayListDiv: props.hiddePlayListDiv,
      isLogged: props.isLogged,
      list: props.list,
      playLists: props.playLists,
      trackFoundInPlayListMessage: props.trackFoundInPlayListMessage
    });
  }
            ...
            <li className="list__container__item" key={i}>
              <div className="list__container__item__group">
                ...
                {this.state.type === "playlist" && (
                  <div>
                    <button
                      onClick={() => this.props.onDeleteClick(item.id)}
                ...
                {this.state.type === "songs" && this.state.isLogged && (
                  <div>
                    <button
                      id={`button-${item.id}`}
                      onClick={() => this.props.onClickAddTrackToList(item.id)}
                      className="list__container__item__group__button-delete btn btn-sm btn-dark"
                    >
                      Add To PlayList
                    </button>
                ...
                >
                  <ul className="list__container__item__playlists-list">
                    {this.state.playLists.map(playlist => (
                      <li
                        onClick={() =>
                          this.props.onPlayListClick(item.id, playlist.id)
                        }
                      >
                        {playlist.name}
                      </li>
                  ...
                  <div className="list__container__item__playlists-message">
                    {this.state.trackFoundInPlayListMessage}
                  </div>
                </div>
              )}
            </li>
