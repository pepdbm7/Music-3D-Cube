...
describe("userlogic", () => {
  beforeEach(() => {
    let user = {
      ...
      token:
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjViZDBiYzU1ZTc3M2JiMDAwOTllMWViNyIsImlhdCI6MTU0MDQ4Njc1NiwiZXhwIjoxNTQwNDkwMzU2fQ.ULJzj6U8uh5_oNrY52kCf_Lwwkl4NxQJayRoj_sSxSM"
    };
  ...
  describe("createPlaylist", () => {
    ...
    it("should fail on already existing playlist", () => {
      return logic.createPlayList(name).catch(err => {
        expect(err.message).to.equal(
          `Exists a play list with the name: ${name}`
        );
      });
    });
    ...
    it("should fail on non-string playlist name", () => {
      return logic.createPlayList().catch(err => {
  ...
  describe("localStorage", () => {
    it("should succeed on user already in localStorage", () => {
      const user = JSON.parse(localStorage.getItem("user"));
      expect(user.id).to.be.an("string");
      expect(user.token).to.be.an("string");
    });
    it("should fail if data not in localStorage", () => {
      const user = {};
      expect(user.id).to.be.an("string");
      expect(user.token).to.be.an("string");
    });
